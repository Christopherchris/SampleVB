...
set "output_exe=%file_dir%\SampleVB.exe"
set "SampleVB=%file_dir%\SampleVB.exe"
@REM set "Ummm=%bin_dir%\UMMM.exe"
@REM set "mt=C:\Work\BuildTools\UMMM\mt.exe"
...
@REM echo Cleanup %file_dir%...
@REM for %%i in ("%file_dir%\*.*") do (if not "%%~nxi"=="build.bat" if not "%%~nxi"=="UcsFPHub.ini" del "%%i" > nul)
@REM rd /s /q "%file_dir%\Shared" 2>&1
@REM mkdir "%file_dir%\Shared"

@REM echo Copy sources from %src_dir%...
@REM for %%i in ("%src_dir%\*.bas";"%src_dir%\*.cls";"%src_dir%\*.frm";"%src_dir%\*.frx";"%src_dir%\*.vbp") do (copy "%%i" "%file_dir%" > nul)
@REM for %%i in ("%src_dir%\Shared\*.bas";"%src_dir%\Shared\*.cls";"%src_dir%\Shared\*.ctl") do (copy "%%i" "%file_dir%\Shared" > nul)

echo Put lines to sources in %file_dir%...
for %%i in ("%file_dir%\*.vbp") do (start "" /w "%SampleVB%" %%i)
...
@REM echo Embedding manifest in %output_exe%...
@REM "%Ummm%" "%file_dir%\UcsFPHub.ini"
@REM "%mt%" -nologo -manifest "%file_dir%\UcsFPHub.ini.manifest" -outputresource:"%output_exe%;1"


echo Code-signing %output_exe%...
%codesign% /d "Sample application" %output_exe%

@REM git checkout -- "%bin_dir%\..\src\Shared\mdJson.bas"
